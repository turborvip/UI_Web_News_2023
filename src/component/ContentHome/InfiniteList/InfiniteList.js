...
function InfiniteList({ state, setState, page, setPage }) {
    ...
    const getData = () => {
        ...
        axios({
            method: 'get',
            url: `http://localhost:1337/listnormalnews?page=${page}`,
            headers: {
    ...
    return (
        <div className={clsx(styles.normalContent, 'row')}>
            <div id='list' className={clsx('col-md-9 col-lg-9 col-xl-9 col-12', styles.itemsNormalContent)}>
                {

                    state.map((item) => (
                        <Link to={'/news/' + item.id} key={item.id} className='text-decoration-none'>
                            <div className={clsx('row', styles.itemNormalContent)}>
                                <div className={clsx('col-5', styles.imgNormalContent)}>
                                    <img className={clsx('img-fluid img-thumbnail ', styles.imageItem)} src={item.image} alt='' />
                                </div>
                ...
                <div className={styles.loadingSpin}>
                    {
                        loadMore && (
                            <div><BeatLoader color={"#1ECFA9"} css={override} loading={loadMore} size={20} /> </div>
                        )
                    }
                </div>
            </div>
            <div className={clsx('col-md-3 col-lg-3 col-xl-3 col-0', styles.allAds)}>
                <div className={styles.imgAds}>
                    <img className={clsx('img-fluid img-thumbnail')} loading='lazy' src='https://znews-photo.zingcdn.me/w660/Uploaded/tpuoayp/2022_06_01/Solo_thumbthumb_master.jpg' alt='' />
                </div>
